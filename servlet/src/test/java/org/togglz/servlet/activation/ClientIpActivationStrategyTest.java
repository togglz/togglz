...
import static org.togglz.servlet.activation.ClientIpActivationStrategyTest.MockRequest.requestWithHeader;
...
public class ClientIpActivationStrategyTest {
    ...
    protected static class MockRequest {
        ...
        private MockRequest(final String remoteAddr) {
            request = mock(HttpServletRequest.class);
            when(request.getRemoteAddr()).thenReturn(remoteAddr);
            HttpServletRequestHolder.bind(request);
        }

        private MockRequest(final String header, final String headerValue) {
            request = mock(HttpServletRequest.class);
            when(request.getHeader(header)).thenReturn(headerValue);
            HttpServletRequestHolder.bind(request);
        }

        public static MockRequest requestFrom(final String remoteAddr) {
            return new MockRequest(remoteAddr);
        }

        public static MockRequest requestWithHeader(final String header, final String headerValue) {
            return new MockRequest(header, headerValue);
        }
    ...
    @Test
    public void shouldBeActiveForHeaderXForwardedFor() {
        assertThat(requestWithHeader("X-Forwarded-For", "192.168.0.1")).isActiveWithParams("192.168.0.1,10.1.2.3");
    }

    @Test
    public void addressParameterProperties() {
        final AddressParameter param = addressParam();
    ...
    @Test
    public void addressParameterShouldBeValidWithMoreAddressesAndRanges() {
        assertTrue(addressParam().isValid("10.1.2.3,10.1.2.0/24,2001:db8:0:0:0:0:0:1,2001:db8::1,2001:db8:0:0:0:0:0:0/24,2001:db8::/24"));
    }

    @Test
    public void addressParameterShouldBeInvalidWithMoreAddressesAndRangesAndOneInvalid() {
        assertFalse(addressParam().isValid("10.1.2.3,10.1.2.0/24,2001:db8:0:0:0:0:0:1,2001:db8::1,2001:db8:0:0:0:0:0:0/24,2001:db8::/24,some-invalid-string/24"));
        assertFalse(addressParam().isValid("10.1.2.3,10.1.2.0/24,2001:db8:0:0:0:0:0:1,2001:db8:::1,2001:db8:0:0:0:0:0:0/24,2001:db8::/24"));
    }
